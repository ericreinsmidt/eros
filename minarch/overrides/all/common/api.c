...
#include <alsa/asoundlib.h>   /* EROS: raw-ALSA output to a BT sink (SDL can't) */
#include <errno.h>
#include <fcntl.h>
#include <math.h>
#include <msettings.h>
...
{
	// EROS: the in-game menu is the only place a font is used; force EROS's
	// bundled menu font regardless of the configured NextUI font.
	if (access("/mnt/SDCARD/eros/menu.ttf", F_OK)==0)
		fontPath = "/mnt/SDCARD/eros/menu.ttf";
...
// EROS: no keymon runs under EROS, so the volume and FN-brightness keys are
// otherwise dead in-game -- minarch only *shows* an indicator when the value
// changes, and nothing changes it. Read them straight off the merged input node
// and apply them here, at the top of every frame (GFX_startFrame runs in both
// the live-game loop and every menu). Changing the value makes minarch's own
// poll raise the system indicator, which EROS draws as a thin top line (see
// render_system_indicator). This is the single source of truth for both
// settings -- brightness is NOT handled in PWR_update anymore (would double-step
// in the menus, where both fire).
//
// The keys come in on /dev/input/event3 ("TRIMUI Player1", the virtual joystick
// trimui_inputd publishes) -- confirmed with getevent while a game was running:
// volume is KEY_VOLUMEUP/DOWN and the FN buttons are BTN_THUMBL/R (317/318).
// event0 (sunxi-keyboard) stays silent in-game, which is why this used to do
// nothing. The device is not grabbed, so this extra reader gets every event.
static void EROS_pollSettingKeys(void)
{
	// evdev event + codes defined locally ON PURPOSE. Do NOT #include
	// <linux/input.h> here: it #defines BTN_A/B/X/Y/START/SELECT to their raw key
	// codes (0x130..), which shadow minarch's BTN_* button-mask enum (defines.h,
	// which even warns about this) and corrupt the entire in-game input map.
	struct eros_ev { long sec, usec; unsigned short type, code; int value; }; // == struct input_event (24 bytes)
	enum { EV_KEY_ = 1,
	       KEY_VOLUP = 115, KEY_VOLDN = 114,   // KEY_VOLUMEUP / KEY_VOLUMEDOWN
	       BTN_FN_L = 317,  BTN_FN_R = 318 };  // BTN_THUMBL / BTN_THUMBR (FN left/right)

	static int fd = -2;                 // -2 unopened, -1 failed
	if (fd == -2) fd = open("/dev/input/event3", O_RDONLY | O_NONBLOCK);
	if (fd < 0) return;

	struct eros_ev ev;
	while (read(fd, &ev, sizeof ev) == (ssize_t)sizeof ev) {
		if (ev.type != EV_KEY_ || ev.value == 0) continue;   // press + repeat, not release
		switch (ev.code) {
		...
		case BTN_FN_R:  { int b = GetBrightness(); if (b < BRIGHTNESS_MAX) SetBrightness(b + 1); } break;
		case BTN_FN_L:  { int b = GetBrightness(); if (b > BRIGHTNESS_MIN) SetBrightness(b - 1); } break;
		default: break;
		}
	}
}
...
// EROS: the one true settings indicator -- a thin line across the very top,
// matching Muse's play bar (6px, faint track + near-white fill) with a
// transparent-black scrim above/below. Replaces MinUI's corner pill. `kind`:
// 1 = brightness, 2 = volume. Drawn onto whatever surface is being composited
// (the menu's `screen`, or the in-game notification layer). No glyph, no text.
void EROS_drawSettingLine(SDL_Surface *dst, int kind)
{
	if (!dst) return;
	int val, mn, mx;
	if (kind == 1) { val = GetBrightness(); mn = BRIGHTNESS_MIN; mx = BRIGHTNESS_MAX; }
	else           { val = GetVolume();     mn = VOLUME_MIN;     mx = VOLUME_MAX; }
	float pct = mx > mn ? (float)(val - mn) / (mx - mn) : 0.f;
	if (pct < 0) pct = 0; else if (pct > 1) pct = 1;

	int W = dst->w;
	int line_h = SCALE1(2);     // 6px at FIXED_SCALE=3, == Muse play bar
	int pad = line_h / 2;       // 3px transparent-black above + below

	SDL_Surface *scrim = SDL_CreateRGBSurfaceWithFormat(0, W, line_h + pad * 2, 32, SDL_PIXELFORMAT_ARGB8888);
	if (scrim) {
		...
		SDL_BlitSurface(scrim, NULL, dst, &(SDL_Rect){ 0, 0 });
		SDL_FreeSurface(scrim);
	}
	SDL_FillRect(dst, &(SDL_Rect){ 0, pad, W, line_h },              SDL_MapRGB(dst->format, 60, 62, 72));
	SDL_FillRect(dst, &(SDL_Rect){ 0, pad, (int)(W * pct), line_h }, SDL_MapRGB(dst->format, 235, 235, 240));
}
...
int GFX_blitHardwareGroup(SDL_Surface *dst, int show_setting)
{
	...
	{
		// EROS: draw the top line instead of MinUI's corner pill. show_setting is
		// 1=brightness, 2=volume, 3=colortemp (ignored -- unused on the Brick).
		if (show_setting == 1 || show_setting == 2)
			EROS_drawSettingLine(dst, show_setting);
		return 0;
	}
	else
	{
...
/* ---- Bluetooth audio output (EROS) --------------------------------------
 * SDL2 cannot drive the bluealsa ALSA plugin: it opens the PCM but its audio
 * thread is never woken by bluealsa's poll fds, so the callback never fires and
 * it hangs (verified on bluealsa v3 and v4). Raw snd_pcm_writei plays fine (as
 * Muse and aplay do). So when the launcher points AUDIODEV at a bluealsa device,
 * SND_init skips SDL and starts this writer thread: it drains the very same ring
 * buffer SND_batchSamples fills and blocking-writes it to the sink. The blocking
 * write paces us at real time and SND_batchSamples' resample feedback keeps the
 * buffer balanced -- exactly like the SDL callback path. Speaker stays pure SDL. */
#ifndef F_SETPIPE_SZ
#define F_SETPIPE_SZ 1031
#endif
static int          bt_mode    = 0;
static int          bt_fd      = -1;   /* FIFO to the aplay forwarder */
static pthread_t    bt_thread;
static volatile int bt_running = 0;
static volatile int bt_paused  = 1;

static void *SND_btThread(void *arg)
{
	(void)arg;
	SND_Frame local[512];
	...
	while (bt_running)
	{
		...
		if (!bt_paused)
		{
			while (snd.frame_out != snd.frame_in && n < 512)
			{
				local[n++] = snd.buffer[snd.frame_out];
				snd.frame_out += 1;
		...
		/* feed silence when empty/paused so the forwarder keeps the a2dp stream
		 * alive; the blocking write to the FIFO paces us at the sink's real rate */
		const char *buf = n > 0 ? (const char *)local : (const char *)silence;
		size_t bytes = (size_t)(n > 0 ? n : 512) * sizeof(SND_Frame);
		while (bytes > 0)
		{
			ssize_t w = write(bt_fd, buf, bytes);
			if (w < 0)
			{
				if (errno == EINTR) continue;
				if (!bt_running) break;
				usleep(2000);   /* forwarder gone/backed up -- back off, don't spin */
				break;
			}
			buf += w; bytes -= (size_t)w;
		}
	}
	return NULL;
}
...
{ // plat_sound_resize_buffer
	...
	// EROS: in BT mode the ring's consumer is our writer thread, not SDL -- guard
	// the realloc with audio_mutex (SDL_LockAudioDevice is a no-op here).
	if (bt_mode) pthread_mutex_lock(&audio_mutex);
#if defined(USE_SDL2)
	else SDL_LockAudioDevice(snd.device_id);
#else
	else SDL_LockAudio();
#endif
	...
	if (bt_mode) pthread_mutex_unlock(&audio_mutex);
#if defined(USE_SDL2)
	else SDL_UnlockAudioDevice(snd.device_id);
#else
...
{ // plat_sound_init
	...
	// EROS: game audio over BT. Neither SDL nor in-process ALSA can drive bluealsa
	// from the game process (both hang). So the launcher runs a standalone `aplay`
	// forwarder that owns the bluealsa sink, and minarch just writes raw PCM to a
	// FIFO (EROS_BT_SINK = the FIFO path). No ALSA/SDL audio in this process at all.
	const char *btsink = getenv("EROS_BT_SINK");
	bt_mode = (btsink && btsink[0]);
	...
	// EROS: BT sink -> write raw PCM to the forwarder FIFO (btsink). SDL was
	// skipped entirely. The resampler feeds a fixed 48k; aplay resamples to the
	// sink. open() blocks until the forwarder opens the FIFO for reading.
	if (bt_mode)
	{
		int rate = 48000;
		bt_fd = open(btsink, O_WRONLY);
		if (bt_fd < 0)
		{
			LOG_info("BT audio: FIFO %s open failed (%s) -- using speaker\n", btsink, strerror(errno));
			bt_mode = 0;
			SDL_InitSubSystem(SDL_INIT_AUDIO);   // fell back: bring up SDL for the speaker path
		}
		else
		{
			fcntl(bt_fd, F_SETPIPE_SZ, 4096);   /* one-page FIFO (~20ms) to keep added latency low */
			snd.device_id = -1;
			snd.sample_rate_in = sample_rate;
			snd.sample_rate_out = rate;
			perf.samplerate_in = snd.sample_rate_in;
			perf.samplerate_out = snd.sample_rate_out;
			snd.frame_count = ((float)rate / SCREEN_FPS) * 8;
			perf.buffer_size = snd.frame_count;
			SND_resizeBuffer();
			bt_running = 1;
			bt_paused = 1;
			pthread_create(&bt_thread, NULL, SND_btThread, NULL);
			snd.initialized = 1;
			LOG_info("BT audio: piping to forwarder via %s @ %dHz\n", btsink, rate);
			return;
		}
	}
...
void SND_quit(void)
{
	...
	if (bt_mode)   /* EROS: stop the writer thread + close the forwarder FIFO */
	{
		bt_running = 0;
		pthread_join(bt_thread, NULL);
		if (bt_fd >= 0) { close(bt_fd); bt_fd = -1; }   /* EOF -> aplay forwarder exits */
		bt_mode = 0;
		snd.initialized = 0;
...
{
	if (bt_mode) { bt_paused = paused ? 1 : 0; return; }   /* EROS: BT writer thread */
#if defined(USE_SDL2)
...
#define CHARGE_DELAY 1000
	...
	// EROS: volume + FN-brightness are handled in one place, EROS_pollSettingKeys
	// (GFX_startFrame), which reads /dev/input/event3 directly and works in-game
	// too. Don't also step brightness here -- in the menus both PWR_update and
	// GFX_startFrame run each frame, which would double-step the level.
...
// EROS: minarch owns State_autosave() (ma_saves.c); declared here so the shared
// power path can quicksave. common/api.c is only linked into minarch, so this
// extern never dangles in another binary.
extern void State_autosave(void);

void PWR_powerOff(int reboot)
{
	if (pwr.can_poweroff)
	{
		// EROS: quicksave to the resume slot (9) before shutting down, so
		// powering off mid-game resumes on the next launch -- same as Quit.
		// This is the one point every power-off path funnels through, so it
		// catches the cases that skip the before_sleep callback.
		State_autosave();
		...
		// EROS: every power-off now quicksaves (State_autosave above), so the game
		// is always waiting for you. Send-off line instead of a status message.
		char *msg = reboot > 0 ? (char *)"Rebooting" : (char *)"We'll always have Paris.";
...
// EROS: this handheld's LEDs stay off (save power, no chrome). Kill the
// led_anim engine and every raw channel directly.
void EROS_ledsOff(void)
{
	int fd;
	const char *zeros[] = {
		"/sys/class/led_anim/effect_enable",
		"/sys/class/led_anim/max_scale",
		"/sys/class/led_anim/max_scale_lr",
		"/sys/class/led_anim/max_scale_f1f2",
	};
	for (size_t i=0;i<sizeof zeros/sizeof *zeros;i++) {
		fd = open(zeros[i], O_WRONLY);
		if (fd>=0) { if (write(fd,"0",1)<0){} close(fd); }
	}
	char path[96];
	for (int n=0;n<23;n++) for (const char *c="rgb"; *c; c++) {
		snprintf(path,sizeof path,"/sys/class/leds/sunxi_led%d%c/brightness",n,*c);
...
{
	// EROS: never light LEDs (including battery indicators).
	(void)indicator_only;
	EROS_ledsOff();
	return;
	if(lights_initialized == 0) {
...
void LEDS_initLeds()
{
	...
	// EROS: never light the LEDs -- kill them instead of applying a profile.
	EROS_ledsOff();
}

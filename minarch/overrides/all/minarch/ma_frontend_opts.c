...
int Menu_messageWithFont(char* message, char** pairs, TTF_Font* f) {
	...
	while (1) {
		...
		GFX_clear(screen);
		GFX_clearLayers(0); // EROS: dark bg, and no button-hint footer below
...
int OptionEmulator_openMenu(MenuList* list, int index) {
	...
	// EROS: only add categories that contain a visible option. Empty ones
	// (e.g. "DIP Switches", populated only by arcade cores) would otherwise
	// open a dead "no options" page.
	int added_cats = 0;
	for (int i=0; i<cat_count; i++) {
		OptionCategory *cat = &config.core.categories[i];
		int has = 0;
		for (int k=0; k<config.core.count; k++) {
			Option *o = &config.core.options[k];
			if (o->lock || o->hidden) continue;
			if (o->category && strcmp(o->category, cat->key)==0) { has = 1; break; }
		}
		if (!has) continue;
		MenuItem *item = &OptionEmulator_menu.items[added_cats++];
		item->key = cat->key;
		item->name = cat->desc;
		item->desc = cat->info;
	}
	...
	for (int i=0; i<config.core.enabled_count; i++) {
		...
		MenuItem *item = &OptionEmulator_menu.items[added_cats + i];
		item->key = option->key;
		item->name = option->name;
		item->desc = option->desc;
		item->value = option->value;
	...
	if (added_cats || config.core.enabled_count) {
...
int OptionControls_openMenu(MenuList* list, int i) {
	...
	if (OptionControls_menu.items==NULL) {
		...
		for (int j=0; config.controls[j].name; j++) {
			...
			// EROS: hide L3/R3 -- the Brick's FN buttons are brightness-only, so
			// these are never mappable (and never reach a core; see ma_input.c).
			if (button->retro==RETRO_DEVICE_ID_JOYPAD_L3 || button->retro==RETRO_DEVICE_ID_JOYPAD_R3) continue;
		...
		for (int j=0; config.controls[j].name; j++) {
			ButtonMapping* button = &config.controls[j];
			if (button->ignore) continue;
			if (button->retro==RETRO_DEVICE_ID_JOYPAD_L3 || button->retro==RETRO_DEVICE_ID_JOYPAD_R3) continue; // EROS: L3/R3 hidden

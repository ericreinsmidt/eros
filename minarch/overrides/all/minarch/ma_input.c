...
void input_poll_callback(void) {
	...
	for (int i=0; config.controls[i].name; i++) {
		...
		// EROS: the Brick's FN buttons are the L3/R3 codes, but EROS owns them for
		// brightness -- never let L3/R3 reach a core. (None of EROS's systems use
		// the analog-stick clicks anyway.) Also removed from the Controls menu.
		if (mapping->retro==RETRO_DEVICE_ID_JOYPAD_L3 || mapping->retro==RETRO_DEVICE_ID_JOYPAD_R3) continue;
		int btn = 1 << mapping->local;
...
void Input_init(const struct retro_input_descriptor *vars) {
	...
	int present[RETRO_BUTTON_COUNT] = {0}; // EROS: was uninitialized (garbage -> wrong ignore)
	int core_mapped = 0;
	if (vars) {
		core_mapped = 1;

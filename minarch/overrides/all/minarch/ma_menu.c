...
// EROS in-game menu styling: translucent white pill with very slightly
// rounded corners, drawn onto the software menu surface.
#define EROS_PILL_ALPHA 56
#define EROS_PILL_RADIUS 6
static void EROS_blitPillAlpha(SDL_Surface* dst, SDL_Rect r, Uint8 alpha) {
	if (r.w <= 0 || r.h <= 0) return;
	SDL_Surface* s = SDL_CreateRGBSurfaceWithFormat(0, r.w, r.h, 32, SDL_PIXELFORMAT_ARGB8888);
	if (!s) return;
	SDL_FillRect(s, NULL, SDL_MapRGBA(s->format, 255, 255, 255, alpha));
	int rad = EROS_PILL_RADIUS;
	if (rad*2 > r.w) rad = r.w/2;
	if (rad*2 > r.h) rad = r.h/2;
	Uint32* px = (Uint32*)s->pixels;
	int stride = s->pitch/4;
	Uint32 clear = SDL_MapRGBA(s->format, 0, 0, 0, 0);
	for (int y=0; y<rad; y++) for (int x=0; x<rad; x++) {
		int dx = rad-x, dy = rad-y;
		if (dx*dx + dy*dy > rad*rad) {
			px[y*stride + x] = clear;
			px[y*stride + (r.w-1-x)] = clear;
			px[(r.h-1-y)*stride + x] = clear;
			px[(r.h-1-y)*stride + (r.w-1-x)] = clear;
		}
	}
...
static void EROS_blitPill(SDL_Surface* dst, SDL_Rect r) { EROS_blitPillAlpha(dst, r, EROS_PILL_ALPHA); }

// Vertically center a text row on its CAP height. TTF_Render* surfaces are the
// full font height (ascent+descent), and most labels have no descenders, so the
// usual box-centering `top + (row_h - surf_h)/2` leaves the empty descent gap at
// the bottom and the text rides high. This places the baseline so the caps
// straddle the row's center. Returns the y for the surface's top. Reads live
// font metrics, so it's correct for any font/size (font.large/small/tiny/...).
static int EROS_textCenterY(TTF_Font* f, int row_top, int row_h) {
	int cap_top = 0;
	TTF_GlyphMetrics(f, 'H', NULL, NULL, NULL, &cap_top, NULL);
	return row_top + (row_h + cap_top) / 2 - TTF_FontAscent(f);
}

// EROS: dim the frozen game frame directly onto the menu surface (opaque),
// so nested menus darken consistently regardless of GL layer state.
static void EROS_dimBg(SDL_Surface* dst, SDL_Surface* bmp) {
	if (bmp) SDL_BlitScaled(bmp, NULL, dst, NULL);
	SDL_Surface* d = SDL_CreateRGBSurfaceWithFormat(0, dst->w, dst->h, 32, SDL_PIXELFORMAT_ARGB8888);
	if (d) {
		SDL_FillRect(d, NULL, SDL_MapRGBA(d->format, 0, 0, 0, 205));
		SDL_SetSurfaceBlendMode(d, SDL_BLENDMODE_BLEND);
		SDL_BlitSurface(d, NULL, dst, NULL);
		SDL_FreeSurface(d);
	}
}
...
void Menu_init(void) {
	...
	// EROS: mkdir(2) only makes the leaf, so the ".minui" parent was never created
	// and every state-preview .bmp write silently failed -> blank slot preview.
	// Create the parent first.
	mkdir(SHARED_USERDATA_PATH "/.minui", 0755);
		...
		if (dirty) {
			...
			for (int j = start, row = 0; j < end && j < filtered_count; j++, row++) {
				...
				if (is_selected) {
					...
					text_color = COLOR_WHITE; // EROS: white on translucent pill
...
// EROS: trimmed in-game options. Frontend->Display (no NextUI subtitle);
// Shaders/Cheats/Shortcuts/Achievements removed. RA stays compiled but is
// disabled (RAEnable=0) and has no menu surface.
static MenuList options_menu = {
	.type = MENU_LIST,
	.items = (MenuItem[]) {
		{"Display",.on_confirm=OptionFrontend_openMenu},
...
static int save_changes_index = 3;

static void Options_updateVisibility(void) {
	save_changes_index = 3;
}
...
// EROS: Achievements row removed; nothing to update.
...
int Menu_options(MenuList* list) {
	...
	while (show_options) {
		...
		GFX_clear(screen);
		// EROS: dimmed frozen game frame, no chrome (matches the pause menu).
		// Clear the game layer first (the pause menu does this once before its
		// loop; nested option menus must do it too or the game shows bright).
		GFX_clearLayers(0);
		if (menu.bitmap) GFX_drawOnLayer(menu.bitmap,0,0,DEVICE_WIDTH,DEVICE_HEIGHT,0.15f,1,0);
		...
		if (type==MENU_LIST) {
			...
			int ox = SCALE1(PADDING); // EROS: left-justified, no values -> widest-item pill
			int oy = SCALE1(PADDING + PILL_SIZE);
			int rh = SCALE1(BUTTON_SIZE);
			int selected_row = selected - start;
			for (int i=start,j=0; i<end; i++,j++) {
				MenuItem* item = &items[i];
				int rtop = oy+SCALE1(j*BUTTON_SIZE);
				text = TTF_RenderUTF8_Blended(font.small, item->name, COLOR_WHITE);
				int ty = EROS_textCenterY(font.small, rtop, rh);
				if (j==selected_row) {
					EROS_blitPill(screen, (SDL_Rect){ox, rtop, mw, rh});
					if (item->desc) desc = item->desc;
				}
				SDL_BlitSurface(text, NULL, screen, &(SDL_Rect){ ox+SCALE1(OPTION_PADDING), ty });
				SDL_FreeSurface(text);
			}
		}
		else if (type==MENU_FIXED) {
			// EROS: full-width pill only when a row shows a right-side value;
			// otherwise the pill is the width of the widest label.
			int has_values = 0;
			for (int i=0;i<count;i++) if (items[i].values) { has_values = 1; break; }
			int mw;
			if (has_values) {
				...
				mw = 0;
				for (int i=0;i<count;i++) {
					int w=0; TTF_SizeUTF8(font.small, items[i].name, &w, NULL);
					w += SCALE1(OPTION_PADDING*2);
					if (w>mw) mw=w;
				}
			}
			...
			for (int i=start,j=0; i<end; i++,j++) {
				...
				if (item->values == NULL) {
					// EROS: navigation item -- no ">" indicator, just the label.
				}
				else {
					if (item->value>=0) {
						int count = 0;
						...
						if (item->value >= 0 && item->value < count) {
							...
							if (text) {
								SDL_BlitSurface(text, NULL, screen, &(SDL_Rect){
									...
									EROS_textCenterY(font.tiny, oy+SCALE1(j*BUTTON_SIZE), SCALE1(BUTTON_SIZE))
								});
								SDL_FreeSurface(text);
							}
						}
				...
				if (j==selected_row && item->desc) desc = item->desc;
				text = TTF_RenderUTF8_Blended(font.small, item->name, text_color);
				SDL_BlitSurface(text, NULL, screen, &(SDL_Rect){
					ox+SCALE1(OPTION_PADDING),
					EROS_textCenterY(font.small, oy+SCALE1(j*BUTTON_SIZE), SCALE1(BUTTON_SIZE))
				});
				SDL_FreeSurface(text);
			}
		}
			...
			// EROS: value rows -> full-width pill, left-justified.
			mw = screen->w - SCALE1(PADDING*2);
			int ox = SCALE1(PADDING);
			int oy = SCALE1(PADDING + PILL_SIZE);
			int selected_row = selected - start;
		...
		// EROS: no bottom description/hint block; nothing but the rows.
		(void)desc;
...
void Menu_saveState(void) {
	...
	State_write();
	// EROS: no save toast -- the device deliberately shows no in-game chrome.
}
void Menu_loadState(void) {
	...
	if (menu.save_exists) {
		...
		// EROS: no load toast (see Menu_saveState).
	}
}
...
void Menu_loop(void) {
	...
	while (show_menu) {
		...
		if (PAD_justPressed(BTN_B) || (BTN_WAKE!=BTN_MENU && PAD_tappedMenu(now))) {
			...
			switch(selected) {
				...
				case ITEM_QUIT:
					...
					// game picks up where you left off -- the launcher writes
					// resume_slot=9 on launch and State_resume() loads it. Without
					// this, Quit left slot 9 stale and "auto-resume" did nothing.
					State_autosave();
					status = STATUS_QUIT;
					show_menu = 0;
					quit = 1; // TODO: tmp?
		...
		if(dirty) {
			...
			// EROS: dimmed frozen game frame, no chrome (no status bar, no
			// game name, no button hints). Just the centered item list.
			GFX_drawOnLayer(menu.bitmap,0,0,DEVICE_WIDTH,DEVICE_HEIGHT,0.15f,1,0);

			int ox, oy;
			SDL_Surface* text;

			// EROS: left-justified on the left of screen (bigger font than the
			// submenus since it's used most). No values here, so the pill is the
			// width of the widest item. The Save/Load preview sits on the right,
			// clear of the left-aligned list.
			int left = SCALE1(PADDING);
			int pad = SCALE1(BUTTON_PADDING);
			int row_h = SCALE1(PILL_SIZE);
			int pill_w = 0;
			for (int i=0; i<MENU_ITEM_COUNT; i++) {
				int w; TTF_SizeUTF8(font.large, menu.items[i], &w, NULL);
				if (w>pill_w) pill_w = w;
			}
			pill_w += pad*2;
			int start_y = (screen->h - MENU_ITEM_COUNT*row_h)/2;

			for (int i=0; i<MENU_ITEM_COUNT; i++) {
				int ry = start_y + i*row_h;
				text = TTF_RenderUTF8_Blended(font.large, menu.items[i], COLOR_WHITE);
				int ty = EROS_textCenterY(font.large, ry, row_h);
				if (i==selected)
					EROS_blitPill(screen, (SDL_Rect){left, ry, pill_w, row_h});
				SDL_BlitSurface(text, NULL, screen, &(SDL_Rect){ left + pad, ty });
				SDL_FreeSurface(text);
			}
			...
			if (selected==ITEM_SAVE || selected==ITEM_LOAD) {
				...
				// window
				// EROS: bezel around the preview = the same soft-white pill as the
				// selected list row (instead of the stock ASSET_STATE_BG window).
				EROS_blitPill(screen, (SDL_Rect){ox,oy,pw,ph});
				...
				for (int i=0; i<MENU_SLOT_COUNT; i++) {
					// EROS: current slot = opaque white (the list's text color), other
					// slots = the soft-white pill alpha. All monochrome, matching the list.
					int d = SCALE1(6);
					Uint8 a = (i==menu.slot) ? 255 : EROS_PILL_ALPHA;
					EROS_blitPillAlpha(screen, (SDL_Rect){ox+SCALE1(i*15), oy, d, d}, a);
				}
			}
			GFX_flip(screen);
			dirty=0;
